...
- Use docstrings to document the contract logic being validated, not implementation trivia.
- For behavior tests, docstrings should state the decision or invariant under test (what must hold true and why it matters).
- For error-path tests, docstrings should name the trigger, expected failure handling/classification, and caller-visible outcome.
- State the implication of failure: what guarantee is lost, what user/operator risk appears, or what downstream behavior becomes unsafe.
- Prefer docstrings whenever test purpose is non-trivial, regression-prone, or not fully clear from the function name.
- Keep docstrings focused on intent, not Arrange/Act details.
- Use the shortest form that still captures contract, error path, and implication:
  - Simple/unit cases: typically 1-3 lines.
  - Contract/reliability/error-path cases: 3-6 lines when needed.
...
Docstring template (adapt as needed):
