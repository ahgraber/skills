...
# dependencies = []
# ///
"""Project canonical security scan JSON into the standard reports."""
...
def _load_script(name: str) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(f"security_scan_{name}", path)
...
def _section_code_evidence(finding: dict[str, Any], section: dict[str, Any]) -> list[dict[str, Any]]:
    catalog = _code_evidence_catalog(finding)
    refs = section.get("evidenceRefs", section.get("evidence_refs", []))
    resolved = (
        [catalog[ref] for ref in refs if isinstance(ref, str) and ref in catalog] if isinstance(refs, list) else []
    )
...
def _root_cause_code_evidence(finding: dict[str, Any], root_cause: dict[str, Any]) -> list[dict[str, Any]]:
...
def _severity_mix(findings: list[dict[str, Any]]) -> str:
    ...
    return ", ".join(f"{level}: {counts[level]}" for level in SEVERITY_ORDER if counts[level]) or "none"


def _confidence_mix(findings: list[dict[str, Any]]) -> str:
    counts = Counter(finding["confidence"]["level"] for finding in findings)
    return ", ".join(f"{level}: {counts[level]}" for level in ("high", "medium", "low") if counts[level]) or "none"
...
def _target_scope_lines(target: dict[str, Any]) -> list[str]:
    ...
    if base_revision or head_revision:
        lines.append(f"- Revision range: {base_revision or 'unknown'}...{head_revision or 'unknown'}")
...
def _finding_section(number: int, finding: dict[str, Any]) -> list[str]:
    ...
    reachability = attack_path.get("reachability") if isinstance(attack_path.get("reachability"), dict) else {}
...
# This assembles many independent report sections in sequence; the branch count
# reflects the document shape, not tangled control flow, so the limit is waived.
def build_report_markdown(  # noqa: C901
    manifest: dict[str, Any], findings_document: dict[str, Any], coverage: dict[str, Any]
) -> str:
    """Render the canonical scan documents into the standard report markdown."""
    scan = manifest["scan"]
    target = scan["target"]
    scope = scan["scope"]
            ...
            lines.append(
                ...
                + " | ".join(
                    (
                        ...
                        _cell(DISPOSITION_LABELS.get(surface.get("disposition"), surface.get("disposition"))),
        ...
        questions.extend(
            {
                ...
                "followUpPrompt": " ".join(
                    (
                        ...
                        (f"Surfaces: {', '.join(item.get('surfaceIds', []))}." if item.get("surfaceIds") else ""),
                    )
                ).strip(),
            }
...
def generate_report_markdown(
    ...
    """Render and validate the report, returning UTF-8 bytes."""
