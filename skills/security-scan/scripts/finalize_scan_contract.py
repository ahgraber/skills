...
# dependencies = []
# ///
"""Validate and seal additive security scan-contract artifacts."""
...
SCHEMA_VERSION = "1.0"
FINGERPRINT_ALGORITHM = "security-scan/v1"
...
def _generate_report_projection(
    ...
    spec = importlib.util.spec_from_file_location("security_scan_report_projection", script)
...
def _validate_scan_local_output_path(scan_dir: Path, path: Path, relative_path: str) -> None:
    ...
    if resolved_parent != path.parent or path.is_symlink():
        raise ContractError(f"{relative_path}: expected a non-symlink path inside the scan directory")
...
def open_scan_local_file_descriptor(scan_dir: Path, relative_path: str, context: str) -> int:
    """Open a verified scan-local file and return a read-only descriptor."""
...
def _read_scan_local_json_bytes(scan_dir: Path, relative_path: str, context: str) -> tuple[dict[str, Any], bytes]:
    descriptor = open_scan_local_file_descriptor(scan_dir, relative_path, context)
    try:
...
def write_scan_local_bytes(scan_dir: Path, relative_path: str, payload: bytes) -> None:
    """Atomically write bytes to a verified scan-local file."""
    scan_dir = _require_scan_directory(scan_dir)
...
def _validate_remote(remote: str, context: str) -> None:
    ...
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ContractError(f"{context}: remote URL must not contain credentials, query, or fragment")
...
def _enrich_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    ...
    target_id = _require_str(_require_dict(scan, "target", "manifest.scan"), "targetId", "scan.target")
    ...
    for index, finding in enumerate(_require_list(findings, "findings", "findings")):
        ...
        if expected_occurrence_id in occurrence_ids:
            raise ContractError(f"{context}: duplicate occurrence identity; use identity.instance to split siblings")
...
# This validates many independent finding fields in sequence; the branch count
# reflects the finding schema's breadth, not tangled control flow.
def _validate_finding(finding: dict[str, Any], context: str) -> None:  # noqa: C901
    ...
    if confidence_level not in CONFIDENCES:
        raise ContractError(f"{context}.confidence.level: unsupported confidence: {confidence_level}")
    ...
    for section_name in ("rootCause", "validation", "attackPath"):
        ...
        if unknown_refs:
            raise ContractError(
                f"{context}.{section_name}.evidenceRefs: unknown code-evidence ids: " + ", ".join(unknown_refs)
            )
    ...
    for index, surface in enumerate(_require_list(coverage, "surfaces", "coverage")):
        ...
        for ref_index, ref in enumerate(receipt_refs):
            ...
            if not normalized_ref.startswith("artifacts/"):
                raise ContractError(f"{context}.receiptRefs[{ref_index}]: expected a file under artifacts/")
            receipt_refs[ref_index] = normalized_ref
            _require_scan_local_file(scan_dir, normalized_ref, f"{context}.receiptRefs[{ref_index}]")
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("documentType") != "security-scan.scan-manifest":
        raise ContractError("manifest.documentType: expected security-scan.scan-manifest")
    ...
    for field in ("includePaths", "excludePaths"):
        ...
        for index, value in enumerate(values):
            ...
            _require_safe_relative_path(value, f"manifest.scan.scope.{field}[{index}]", allow_dot=True)
    ...
    for index, artifact in enumerate(artifacts):
        ...
        if not isinstance(artifact, dict):
            raise ContractError(f"{context}: expected an object")
        path = _require_safe_relative_path(_require_str(artifact, "path", context), f"{context}.path")
    ...
    for required_path in ("findings.json", "coverage.json"):
        if required_path not in artifact_paths:
            raise ContractError(f"manifest.scan.artifacts: missing required artifact: {required_path}")


def _validate_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    if findings.get("documentType") != "security-scan.findings":
        raise ContractError("findings.documentType: expected security-scan.findings")
...
# This is a self-contained validator for the JSON Schema subset the contract
# uses; its branch count is inherent to the keywords it supports.
def _validate_schema_node(value: Any, schema: dict[str, Any], context: str) -> None:  # noqa: C901
...
# This ports GitHub's rolling-hash line-fingerprint algorithm; its closures
# share mutable state and cannot be split without obscuring the algorithm.
def _github_line_hashes(  # noqa: C901
    handle: TextIO,
...
def _open_source_file(source_root: Path, relative_path: str) -> TextIO | None:
    ...
    try:
        file_fd = open_scan_local_file_descriptor(source_root, relative_path, f"source file {relative_path}")
...
def _github_line_hash_cache(
    ...
    for relative_path, requested_lines in requested_lines_by_path.items():
        line_hashes = (
            None
            if source_read_budget[0] <= 0
            else _github_line_hashes_for_source(source_root, relative_path, requested_lines, source_read_budget)
        )
        source_path = source_root / relative_path
        for line_number in requested_lines:
            line_hash_cache[(source_path, line_number)] = None if line_hashes is None else line_hashes.get(line_number)
...
def _sarif_result(
    ...
    partial_fingerprints = {
        "securityScan/v1": finding["fingerprints"]["primary"],
    }
...
def build_sarif(manifest: dict[str, Any], findings: dict[str, Any], source_root: Path | None = None) -> dict[str, Any]:
    """Build the SARIF projection for a sealed scan."""
    scan = manifest["scan"]
    target = scan["target"]
    ...
    run: dict[str, Any] = {
        ...
        "properties": {
            "securityScanSchemaVersion": manifest["schemaVersion"],
            "securityScanTargetKind": target["kind"],
        },
    }
...
def _validate_sealed_coverage_receipts(scan: dict[str, Any], coverage: dict[str, Any]) -> None:
    artifact_paths = {
        _require_safe_relative_path(artifact["path"], "sealed artifact path") for artifact in scan["artifacts"]
    }
...
def _validate_existing_seal(
    ...
    *,
    ...
    for index, artifact in enumerate(artifacts):
        ...
        actual_sha256 = (
            _sha256_bytes(contents) if contents is not None else _sha256_scan_local_file(scan_dir, path, context)
        )
        if actual_sha256 != expected_sha256:
            raise ContractError(f"{context}: sealed artifact changed or is missing")


def write_sarif_projection(scan_dir: Path, source_root: Path | None = None, schema_dir: Path | None = None) -> None:
    """Validate a sealed scan and write its SARIF projection."""
    scan_dir = _require_scan_directory(scan_dir)
    ...
    findings, findings_bytes = _read_scan_local_json_bytes(scan_dir, scan["findingsRef"], scan["findingsRef"])
    coverage, coverage_bytes = _read_scan_local_json_bytes(scan_dir, scan["coverageRef"], scan["coverageRef"])
...
def finalize_scan(
    ...
    *,
    ...
    """Validate, seal, and project a completed scan."""
    scan_dir = _require_scan_directory(scan_dir)
    schema_dir = schema_dir or Path(__file__).resolve().parent.parent / "schemas"
    manifest = _read_scan_local_json(scan_dir, "scan-manifest.json", "scan-manifest.json")
    scan = _require_dict(manifest, "scan", "manifest")
    _validate_contract_refs(scan)
    findings, findings_input_bytes = _read_scan_local_json_bytes(scan_dir, scan["findingsRef"], scan["findingsRef"])
    coverage, coverage_input_bytes = _read_scan_local_json_bytes(scan_dir, scan["coverageRef"], scan["coverageRef"])
    ...
    if expected_coverage_mode is not None and coverage.get("mode") != expected_coverage_mode:
        raise ContractError(f"coverage.mode: must match selected scan mode {expected_coverage_mode}")
    ...
    scan["artifacts"] = [
        ...
        *[_artifact_record(scan_dir, ref, "application/octet-stream") for ref in _coverage_receipt_refs(coverage)],
    ]
    _validate_sealed_coverage_receipts(scan, coverage)
    _validate_manifest(manifest)
    validate_against_schema(manifest, schema_dir / "scan-manifest.schema.json")
