...
# dependencies = []
# ///
"""Generate and post-process security scan worklists.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="security scan worklist helper.")
...
def preview_for(path: Path, preview_bytes: int) -> tuple[str, bool]:
    """Return a short text preview and whether the file is binary."""
    try:
    ...
    if is_binary_sample(sample):
        return "", True
    preview = data[:preview_bytes].decode("utf-8", errors="ignore").replace("\n", " ").replace("\r", " ")
    return preview, False


def path_is_excluded(path: Path) -> bool:
    """Return True when a repository-relative path is out of scope."""
...
def resolve_scope(repo: Path, scope: str) -> Path:
    """Resolve and validate a scan scope inside the repository."""
...
def load_jsonl(path: Path, label: str, validator: RowValidator) -> list[JsonRow]:
    """Load and validate rows from a JSONL file."""
    if not path.exists():
...
def require_exact_fields(row: JsonRow, expected: set[str], path: Path, line_number: int) -> None:
    """Require a row to contain exactly the expected fields."""
    actual = set(row)
    if actual != expected:
...
def require_string(row: JsonRow, field: str, path: Path, line_number: int, *, allow_empty: bool) -> None:
    """Require a row field to be a string."""
    value = row[field]
...
def require_unique_paths(rows: list[JsonRow], label: str) -> None:
    """Require every row to have a unique path."""
    seen: set[str] = set()
...
def make_repo_rank_input(args: argparse.Namespace) -> None:
    """Build the repository or scoped-path rank input worklist."""
...
def run_git_changed_paths(repo: Path, diff_args: list[str]) -> list[Path]:
    """Return existing changed files for a Git diff invocation."""
...
def git_changed_paths(repo: Path, base: str, head: str, mode: str) -> list[Path]:
    """Return changed files for the selected diff mode."""
    if mode == "revisions":
...
def make_diff_rank_input(args: argparse.Namespace) -> None:
    """Build the diff-scoped rank input worklist."""
    repo = Path(args.repo).expanduser().resolve()
    if not repo.is_dir():
        raise SystemExit(f"Repo path not found: {repo}")
...
def make_rank_shards(args: argparse.Namespace) -> None:
    """Partition the rank input into deterministic worker shards."""
    if args.max_rows < 1:
...
def validate_rank_shard(input_shard: Path, output_shard: Path) -> tuple[list[JsonRow], list[JsonRow]]:
    """Validate a worker output shard against its input shard."""
    ...
    if expected_paths != actual_paths:
        ...
        raise SystemExit(f"{output_shard}: paths do not match its input shard; missing={missing}; unknown={unknown}")
...
def validate_rank_shard_command(args: argparse.Namespace) -> None:
    """Validate one worker shard from CLI arguments."""
...
def merge_rank_outputs(args: argparse.Namespace) -> None:
    """Validate worker shard outputs and merge them into rank output."""
    rank_input = Path(args.rank_input).expanduser()
    ...
    expected_output_names = {path.name.replace(".input.jsonl", ".output.jsonl") for path in input_shards}
    ...
    for input_shard in input_shards:
        output_shard = input_shard.with_name(input_shard.name.replace(".input.jsonl", ".output.jsonl"))
...
def copy_deep_review_input(args: argparse.Namespace) -> None:
    """Copy every candidate into the deep-review worklist."""
    rank_input = Path(args.rank_input).expanduser()
    rows = load_jsonl(rank_input, "Rank input", validate_rank_input_row)
    require_unique_paths(rows, "Rank input")
...
def select_deep_review_input(args: argparse.Namespace) -> None:
    """Select top-ranked rows for the deep-review worklist."""
...
def main() -> None:
    """Dispatch the worklist subcommand."""
    args = parse_args()
