...
# dependencies = []
# ///
"""Validate the shared security scan final report shape."""
...
def top_level_headings(text: str) -> list[str]:
    """Return all level-1 and level-2 headings."""
...
def validate_required_sections(text: str, errors: list[str]) -> None:
    """Validate the required top-level report sections and their order."""
...
def finding_body(text: str, finding: re.Match[str], next_finding: re.Match[str] | None) -> str:
    """Return the markdown body for a single finding."""
    start = finding.end()
...
def validate_finding_metadata(body: str, line: int, errors: list[str]) -> None:
    """Validate a finding's metadata table fields."""
    first_subsection = body.find("\n#### ")
    metadata_region = body[: first_subsection if first_subsection != -1 else len(body)]
    fields = {match.group(1).strip().rstrip(":") for match in FIELD_ROW_RE.finditer(metadata_region)}
...
def validate_finding_subsections(body: str, line: int, errors: list[str]) -> None:
    """Validate a finding's required subsections and their order."""
    positions: list[int] = []
...
def validate_findings(text: str, errors: list[str]) -> None:
    """Validate finding entries and their numbering."""
...
def validate_report(text: str) -> list[str]:
    """Validate the report and return collected errors."""
    errors: list[str] = []
...
def main() -> int:
    """Run the report-format validator CLI."""
    parser = argparse.ArgumentParser(description="Validate the security scan report format.")
